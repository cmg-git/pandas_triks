...
Level 1 looping:
    Define o function with the logic for rewarding for each row (i.e., person).
...
create a new column with their favorite food of hate food as a reward.
...
for index, person in df.iterrows():
    df.loc[index, 'reward'] = reward(person)
...
condition = ((df['time_in_bed'] > 1
              ) & (df['percent_sleeping'] > 0.1)
             ) | (person['age'] >= 90)

df['reward'] = df['hate_food']
df.loc[condition, 'reward'] = df['favorite_food']
